...
if ($action === 'insert') {
    ...
    if ($company_name && $company_image) {
        $image_name = time() . '_' . basename($company_image['name']);
        $target_dir = '../company/';
        $target_file = $target_dir . $image_name;

        if (move_uploaded_file($company_image['tmp_name'], $target_file)) {
            $stmt = $pdo->prepare("INSERT INTO company (company_name, company_image) VALUES (:company_name, :company_image)");
            $stmt->execute([
    ...
    if ($id && $company_name) {
        ...
        $stmt = $pdo->prepare("UPDATE company SET company_name = :company_name, company_image = :company_image WHERE id = :id");
        $stmt->execute([
            ':company_name' => $company_name,
            ':company_image' => $image_name,
            ':id' => $id,
